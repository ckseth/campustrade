      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 text-center md:text-left">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="w-full lg:w-72 flex-shrink-0">
            <div className="sticky top-28 space-y-8 bg-white/70 backdrop-blur-2xl p-8 rounded-[2.5rem] shadow-[0_8px_40px_rgba(99,102,241,0.06)] border border-white/40 animate-fade-in-up">
              {/* Subtle glow on top */}
              ...
              <div className="flex items-center justify-between">
                <h3 className="text-2xl font-black text-slate-900 flex items-center gap-2">
                  <div className="w-8 h-8 bg-indigo-50/80 rounded-lg flex items-center justify-center">
                ...
                <button 
                  ...
                  className="text-xs font-bold text-slate-400 hover:text-indigo-600 transition-colors bg-slate-50/60 px-3 py-1.5 rounded-lg hover:bg-indigo-50/60"
                >
                  Reset
                </button>
              </div>
                ...
                <div className="space-y-3">
                  {categories.map(cat => (
                    <label key={cat} className="flex items-center group cursor-pointer">
                      ...
                      <span className="ml-3 text-sm font-bold text-slate-600 group-hover:text-indigo-600 transition-colors line-clamp-1">
                        {cat}
                      </span>
                    </label>
                  ))}
                  ...
                  {['Any', 'Sell', 'Rent'].map(type => (
                    <button
                      ...
                      className={`px-4 py-2.5 rounded-xl text-xs font-black border transition-all duration-300 ${
                        ...
                        : 'bg-white/60 backdrop-blur-sm border-slate-200/40 text-slate-500 hover:border-indigo-300 hover:text-indigo-600'
                      }`}
                    >
                      {type}
                    </button>
