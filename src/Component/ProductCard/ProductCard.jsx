      ...
      <div className="card-premium h-full flex flex-col overflow-hidden relative">
          ...
          {showDeleteConfirm && (
            ...
            >
              <motion.div
                ...
                animate={{ scale: 1 }}
                exit={{ scale: 0.8 }}
                className="bg-white/95 backdrop-blur-xl rounded-3xl p-6 text-center shadow-2xl max-w-xs w-full border border-white/40"
              >
                <div className="w-14 h-14 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <Trash2 className="w-7 h-7 text-red-500" />
                </div>
                <h3 className="text-lg font-black text-slate-900 mb-2">Delete Listing?</h3>
                <p className="text-sm text-slate-500 mb-6">This action cannot be undone. The listing will be permanently removed.</p>
                <div className="flex gap-3">
                  <button
                    onClick={handleDeleteCancel}
                    className="flex-1 px-4 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold text-sm hover:bg-slate-200 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
        ...
        <div className="relative aspect-[4/5] overflow-hidden bg-gradient-to-br from-slate-100 to-slate-50">
          ...
          <div className="absolute top-4 left-4 flex flex-col gap-2">
            <span className="badge-indigo bg-white/80 backdrop-blur-xl border-0 shadow-md capitalize">
              {category || 'General'}
            </span>
            {isOwner && (
          ...
          <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0">
            <button
              ...
              className={`w-10 h-10 bg-white/70 backdrop-blur-xl rounded-full flex items-center justify-center transition-all duration-200 border border-white/40 shadow-lg hover:scale-110 ${isWishlisted ? 'bg-red-50/80' : 'hover:bg-white/90'}`}
              title="Wishlist"
            >
              <Heart className={`w-5 h-5 transition-colors ${isWishlisted ? 'text-red-500 fill-red-500' : 'text-slate-700 hover:text-red-500'}`} />
            </button>
            <button
              onClick={handleShareToggle}
              className="w-10 h-10 bg-white/70 backdrop-blur-xl rounded-full flex items-center justify-center hover:bg-white/90 transition-all duration-200 border border-white/40 shadow-lg hover:scale-110"
              title="Share"
            >
          ...
          {showShareMenu && (
            <div 
              className="absolute top-20 right-4 z-20 bg-white/90 backdrop-blur-2xl rounded-2xl shadow-[0_20px_60px_rgba(99,102,241,0.15)] border border-white/40 overflow-hidden min-w-[180px] animate-slide-down"
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); }}
            >
              <button
                onClick={handleCopyLink}
                className="w-full px-4 py-3 text-left text-sm font-bold text-slate-700 hover:bg-indigo-50/60 hover:text-indigo-600 transition-colors flex items-center gap-3"
              >
              ...
              <button
                ...
                className="w-full px-4 py-3 text-left text-sm font-bold text-slate-700 hover:bg-emerald-50/60 hover:text-emerald-600 transition-colors flex items-center gap-3"
              >
          ...
          <div className="absolute bottom-4 left-4 right-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-400">
            <div className="bg-white/60 backdrop-blur-2xl rounded-2xl p-3 flex justify-between items-center border border-white/40 shadow-xl">
              <div>
        ...
        <div className="p-6 flex-1 flex flex-col relative z-10">
          <div className="mb-3 flex justify-between items-start">
            <h3 className="text-xl font-bold text-slate-800 line-clamp-1 group-hover:text-indigo-600 transition-colors duration-300">
              {name}
            </h3>
          </div>
          ...
          <div className="pt-4 border-t border-slate-100/60 flex items-center justify-between mt-auto">
            ...
            <span className="text-sm font-bold text-indigo-600 bg-indigo-50/60 px-2.5 py-1 rounded-lg">{availability === 'Rent' ? 'For Rent' : 'For Sale'}</span>
          </div>

          {/* Owner Actions — Always visible */}
          {isOwner && (
            <div className="mt-4 pt-4 border-t border-slate-100/60 flex gap-3">
              <button
                onClick={handleEdit}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-indigo-50/60 text-indigo-600 font-bold text-sm hover:bg-indigo-100 transition-all duration-200 hover:scale-[1.02]"
              >
